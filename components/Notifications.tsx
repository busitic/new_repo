...
const Notifications = () => {
   ...
   const unreadNotifictions = inboxNotifications.filter((Notifications)=>
  ...
  {count > 0 && (
    <div className='absolute right-2 top-2 z-20 size-2
  ...
  <PopoverContent align='end' className='shad-popover'>
    <LiveblocksUIConfig overrides={{
      ...
      <InboxNotificationList>
        {unreadNotifictions.length <= 0 && (
          <p className='py-2 text-center text-dark-500'>No new Notifications.</p>
        ) }

        {unreadNotifictions.length > 0 && unreadNotifictions.map((
       notification) => (
        <InboxNotification 
        key={notification.id}
